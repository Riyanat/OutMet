	...
	public String toDotString() {
		...
		for (Node<E> node : nodes) {
			if (groupedNodes.containsKey(node.getLabel())) {
				...
				groupedNodes.get(node.getLabel()).setWeight(
		...
		for (Edge<E> edge : edges) {
			if (dotEdges.containsKey(edge.getLabel())) {
				...
				dotEdges.get(edge.getLabel()).setWeight(
						e.getWeight() + edge.getWeight());
			} else {
				Edge<E> dotEdge = new Edge<E>();
				dotEdge.setKey(groupedNodes.get(edge.getSource().getLabel())
						.getKey()
						+ "->"
						+ groupedNodes.get(edge.getTarget().getLabel())
								.getKey());
